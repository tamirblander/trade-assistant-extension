...
async function analyzeScreenshot(dataUrl) {
  ...
  try {
        ...
        messages: [
          {
            role: "user",
            content: [
              { type: "text", text: "Is this a stock or crypto chart? If so, provide a short technical analysis and forecast. If not, say 'No chart detected.'" },
    ...
    const analysis = result.choices?.[0]?.message?.content || "No response from OpenAI.";
    createOverlay(analysis);
  } catch (e) {
    createOverlay("Error analyzing chart: " + e.message);
  }
}
