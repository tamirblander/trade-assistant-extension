// Ensure the script runs only once per page
if (typeof window.tradeAssistantContentScriptInjected === 'undefined') {
    window.tradeAssistantContentScriptInjected = true;
    ...
    chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
      ...
      // # Handle analysis results from background script
        ...
        // The results window is now opened by the background script,
        // so the content script doesn't need to create an overlay.
        // We can still log the result for debugging.
        if (msg.success) {
          console.log("Analysis complete:", msg.analysis);
        } else {
          console.error("Analysis failed:", msg.error);
        }
        sendResponse({success: true});
      }
